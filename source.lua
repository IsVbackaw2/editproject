...
local settingsTable = {
	General = {
		...
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'Vback Keybind'},
		-- buildwarnings
		-- rayfieldprompts
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("Vback | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		...
		if useStudio then
			file = [[
	{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"Vback Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Vback Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
]]
		end
		...
		-- If no settings saved, apply overridden settings only
			...
			for settingName, settingValue in overriddenSettings do
				...
				assert(#split == 2, "Vback | Invalid overridden setting name: " .. settingName)
		...
		local ok, err = pcall(function()
			...
			for id, _ in assetFiles do
				...
				if success then
					...
					warn("Vback | Failed to load custom asset: "..tostring(id).." - "..tostring(asset))
				end
			end
		end)

		if not ok then
			warn("Vback | Failed to load custom assets: "..tostring(err))
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Vback | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Vback | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, Flag in pairs(RayfieldLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Vback | Unable to find '"..FlagName.. "' in the save file.")
...
function RayfieldLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Vback | Not seeing your actions in notifications?')
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Vback Settings', 0, true)

	
	if TabList['Vback Settings'] then
		TabList['Vback Settings'].LayoutOrder = 1000
	end

	if Elements['Vback Settings'] then
		Elements['Vback Settings'].LayoutOrder = 1000
	end
	...
	local configInput = newTab:CreateInput({
		Name = 'Config Name',
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if Settings.LoadingTitle ~= "Vback Premium Hub" then
	...
	if (Settings.KeySystem) then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Vback | "..Key.." Error " ..tostring(Response))
		...
		if not Passthrough and secureMode then
			warn("Vback | Secure Mode: Key system requires a valid saved key. The key UI cannot be shown as it requires loading detectable assets.")
			Rayfield.Enabled = false
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Vback | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Rayfield specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Vback | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Rayfield specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Vback | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Rayfield specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Vback | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Rayfield specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Vback | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Rayfield specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Vback | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Rayfield specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Vback Settings'])
		end)
	end)
...
if useStudio then
	...
	--	LoadingTitle = "Vback Premium Hub",
	--	Theme = 'Default',
	--	Icon = 0,
	--	LoadingSubtitle = "by Vbackaw",
	--	ConfigurationSaving = {
	--		Enabled = true,
